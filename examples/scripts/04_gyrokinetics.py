...
* ``pg.gyrokinetics.available_quantities()`` lists the registered quantity names.
* ``pg.gyrokinetics.load_quantity(quantity, species, name, frame, path=...)`` resolves
...
* ``pg.gyrokinetics.load_distf(name=..., species=..., frame=..., ...)`` reconstructs a
  full distribution function from the saved ``Jf``-times-Jacobian(s) files
  (what the CLI's ``gyrokinetics-load-distf`` command wraps).
...
print("registered quantities:", pg.gyrokinetics.available_quantities())
...
m0, = pg.gyrokinetics.load_quantity(
...
m1, = pg.gyrokinetics.load_quantity(
...
jacobtot_inv, = pg.gyrokinetics.load_quantity(
...
distf = pg.gyrokinetics.load_distf(
